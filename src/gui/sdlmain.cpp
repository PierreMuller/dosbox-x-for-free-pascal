...
bool DOSBOX_parse_argv() {
    ...
    while (control->cmdline->GetOpt(optname)) {
        ...
        if (optname == "v" || optname == "ver" || optname == "version") {
            ...
            fprintf(stderr,"DOSBox-X Free Pascal testsuite adapttation: PierreMuller (from the Free Pascal Core Team)\n\n");
...
int main(int argc, char* argv[]) SDL_MAIN_NOEXCEPT {
    ...
    if (!strcasecmp(imestr, "auto")) {
        ...
        LOG(LOG_MISC,LOG_NORMAL)("DOSBox-X Free Pascal testsuite adapttation: PierreMuller (from the Free Pascal Core Team)");
